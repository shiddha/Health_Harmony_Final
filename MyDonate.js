...
import { takerProfile, giverProfile } from "../../../API/Profile";
function MyDonate() {
  ...
  useEffect(() => {
    getUserDonateItems(user?.email).then((data) => setData(data));
  }, [user, reload]);
  ...
  const handleAccept = (id, caution) => {
    ...
    takerProfile(id, caution, "deduct");
    setTimeout(() => {
      setReload(!reload);
    }, 800);
  };
  ...
  const handlePinSubmit = (id, caution, userEmail) => {
    ...
    donateItemStatus(id, setStatus);
    takeStatus(id, setStatus);
    takerProfile(id, caution, "return");
    giverProfile(userEmail, caution);
    setTimeout(() => {
      setReload(!reload);
    }, 800);
  };
              ...
              {data.map((item) => (
                ...
                >
                  ...
                  <div className="px-3 pt-4">
                    ...
                    <p className="font-bold text-xl">{item.how_many}</p>
